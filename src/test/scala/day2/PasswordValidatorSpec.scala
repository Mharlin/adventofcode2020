...
class PasswordValidatorSpec extends AnyFlatSpec with Matchers {
  val largeInput = """1-13 r: gqdrspndrpsrjfjx
                     ...
                     |6-7 c: cccccdqcc
                     |2-6 x: vjkxbrfwnj
                     |16-18 s: kssssssswssssssssssb""".stripMargin

  "Password validator" should "count the valid password rules with example inputs" in {
    ParseInput
      .parse("""1-3 a: abcde
                       |1-3 b: cdefg
                       |2-9 c: ccccccccc""".stripMargin)
      .pipe(ValidatePassword.validatePasswords(_) shouldEqual 2)
  }

  "Password validator" should "count the valid password rules with large inputs" in {
    ParseInput
      .parse(largeInput)
      .pipe(ValidatePassword.validatePasswords(_) shouldEqual 600)
  }

  "Password validator with updated rule" should "count the valid password rules with example inputs" in {
    ParseInput
      .parse("""1-3 a: abcde
               |1-3 b: cdefg
               |2-9 c: ccccccccc""".stripMargin)
      .pipe(ValidatePassword.validatePasswordsUpdatedRule(_) shouldEqual 1)
  }

  "Password validator with updated rule" should "count the valid password rules with one inputs" in {
    ParseInput
      .parse("""2-9 c: ccccccccc""".stripMargin)
      .pipe(ValidatePassword.validatePasswordsUpdatedRule(_) shouldEqual 1)
  }

  "Password validator with updated rule" should "count the valid password rules with large inputs" in {
    ParseInput
      .parse(largeInput)
      .pipe(ValidatePassword.validatePasswordsUpdatedRule(_) shouldEqual 245)
  }
}
